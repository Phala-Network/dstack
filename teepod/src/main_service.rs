...
use teepod_rpc::{
    ...
    UpgradeAppRequest, VmConfiguration, GetInfoResponse, ResizeVmRequest,
};
use tracing::warn;
...
impl TeepodRpc for RpcHandler {
    ...
    async fn get_info(self, request: Id) -> Result<GetInfoResponse> {
        if let Some(vm) = self.app.get_vm(&request.id) {
            Ok(GetInfoResponse {
                found: true,
                info: Some(vm),
            })
    ...
    async fn resize_vm(self, request: ResizeVmRequest) -> Result<()> {
        ...
        if vm.status != "stopped" {
            return Err(anyhow::anyhow!("vm should be stopped before resize: {}", request.id));
        }
        let work_dir = self.app.config.run_path.join(&request.id);
        let vm_work_dir = VmWorkDir::new(&work_dir);
        let mut manifest = vm_work_dir
            .manifest()
